...
export default function UserProfileClientPage() {
    ...
    const handlePlayPreview = async (track: any) => {
        ...
        // If clicking same track, just stop (toggle)
        ...
        setPlayingTrackTitle(track.name); // Default

        try {
            // 1. Try Lookup by ID
            let previewUrl = track.preview_url;

            const res = await fetch(`https://itunes.apple.com/lookup?id=${track.id}`);
            const data = await res.json();

            if (data.results?.[0]) {
                if (data.results[0].trackName) setPlayingTrackTitle(data.results[0].trackName);
                if (!previewUrl && data.results[0].previewUrl) previewUrl = data.results[0].previewUrl;
            }

            // 2. Fallback: Search by Name + Artist
            if (!previewUrl) {
                console.log("Lookup failed, trying search fallback...");
                const query = encodeURIComponent(`${track.name} ${track.artist || ''}`);
                const searchRes = await fetch(`https://itunes.apple.com/search?term=${query}&entity=song&limit=1`);
                const searchData = await searchRes.json();
                if (searchData.results?.[0]) {
                    if (searchData.results[0].trackName) setPlayingTrackTitle(searchData.results[0].trackName);
                    if (searchData.results[0].previewUrl) previewUrl = searchData.results[0].previewUrl;
                }
            }
            ...
            if (previewUrl) {
                ...
                alert("Pas d'extrait disponible pour ce titre.");
                setPlayingTrackId(null);
                setPlayingTrackTitle(null);
            }
        } catch (e) {
                        ...
                        <div className="flex flex-col items-center md:flex-row md:items-center gap-3 md:gap-4">
                            ...
                            {currentUser && currentUser.id !== profile.id && (
                                <button
                                    ...
                                    className={`px-3 md:px-4 py-1 md:py-1.5 rounded-full border border-white/10 bg-white/5 hover:bg-white/10 backdrop-blur-md text-[10px] md:text-xs font-bold uppercase tracking-widest transition text-white/60 hover:text-white ${isFollowing
                    ...
                    <section className="mb-12 md:mb-20">
                        ...
                        <div className="flex gap-3 md:gap-6 overflow-x-auto pb-2 scrollbar-hide">
                            {profile.top_songs.slice(0, 5).map((item: any, i: number) => (
                                <div
                                    key={i}
                                    className="group relative flex-shrink-0 pt-3 pr-3 cursor-pointer"
                                    onClick={() => handlePlayPreview(item)} // AJOUTER LE CLIC
                                >
                                    {/* Composant Vinyl animé si lecture en cours */}
                                    <div className={playingTrackId === item.id ? "animate-[spin_4s_linear_infinite]" : ""}>
                                        <Vinyl imageUrl={item.image} size="w-32 h-32 md:w-40 md:h-40" />
                                    </div>

                                    {/* Badge de position (inchangé) */}
                                    ...
                                    {/* Titre superposé quand ça joue */}
                                    {playingTrackId === item.id && (
                                        <div className="absolute inset-0 flex items-center justify-center z-40 pointer-events-none">
                                            <div className="w-full h-full flex flex-col items-center justify-center p-2 text-center animate-in fade-in zoom-in duration-300">
                                                <span className="text-xl md:text-2xl animate-pulse mb-1">🎵</span>
                                                <p className="text-[#00e054] text-[10px] md:text-xs font-bold line-clamp-2 leading-tight drop-shadow-md">
                                                    {playingTrackTitle || item.name}
                                                </p>
                                            </div>
                                        </div>
                                    )}
